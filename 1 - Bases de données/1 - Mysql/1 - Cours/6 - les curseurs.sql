...
begin
	...
	open c1;
		b1: loop
			...
			select sec_to_time(sum(time_to_sec(timediff(datea, dated))))  into t from vol where numpil = id;
            update pilote set nbhv = t where numpilote = id;
		end loop b1;
	close c1;
end $$
delimiter ;
...
select * from vol where numpil = 3;
	select sec_to_time(sum(time_to_sec(timediff(datea, dated))))  from vol where numpil = 3;